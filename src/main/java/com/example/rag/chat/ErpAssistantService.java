...
public class ErpAssistantService {
    ...
    /** 模型调用超时执行器。 */
    private final ModelCallTimeout modelCallTimeout;
     ...
     * @param memoryProperties      会话记忆 Token 预算配置
     * @param modelCallTimeout      模型调用超时执行器
     */
    public ErpAssistantService(AssistantClientProvider clientProvider,
        ...
        // 会话窗口大小由配置控制，具体查询上限由 JDBC 仓库控制。
        this.chatMemory = MessageWindowChatMemory.builder()
            .chatMemoryRepository(chatMemoryRepository)
            .maxMessages(this.memoryProperties.getMaxHistoryMessages())
            .build();
    }
    ...
    public ChatAnswerResult ask(String question, String conversationId, String modelId,
        ...
        try {
            ...
            ChatResponse response = this.modelCallTimeout.call(() -> this.clientProvider.resolveClient(modelId).prompt()
            ...
            response = this.businessDataTurnGuard.ensureNonStreaming(response,
                ...
                () -> this.modelCallTimeout.call(() -> this.clientProvider.resolveClient(modelId).prompt()
                    .options(ChatOptions.builder().model(modelName))
                    .toolContext(this.lifecycleService.buildToolContext(
                        traceId, conversationId, mode, modelName))
                    .advisors(advisor -> advisor
    ...
    public ChatAnswerResult askKnowledge(String question, String conversationId, String modelId,
        ...
        try {
            ...
            ChatResponse response = this.modelCallTimeout.call(() -> noToolsClient.prompt()
                .options(ChatOptions.builder().model(modelName))
                .toolContext(this.lifecycleService.buildToolContext(
                    traceId, conversationId, mode, modelName))
                .advisors(advisor -> advisor
