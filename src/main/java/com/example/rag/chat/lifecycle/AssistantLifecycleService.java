...
public class AssistantLifecycleService {
	...
	/** 模型调用超时配置。 */
	private final AiTimeoutProperties aiTimeoutProperties;
	 ...
	 * @param aiTimeoutProperties 模型调用超时配置
	 */
	public AssistantLifecycleService(ChatHistoryService chatHistoryService,
			...
			AiTimeoutProperties aiTimeoutProperties) {
		...
		this.aiTimeoutProperties = aiTimeoutProperties;
	}
	...
	public Flux<ChatStreamFrame> recordStream(Flux<ChatResponse> responseFlux, String question,
		...
		Flux<ChatStreamFrame> deltaFrames = responseFlux
			.timeout(this.aiTimeoutProperties.getStreamTimeout())
			.doOnNext(response -> {
